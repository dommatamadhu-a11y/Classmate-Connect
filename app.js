...
onAuthStateChanged(auth,user=>{
  if(user){
    ...
    loadUsersMap();  // Load users map for chats
    loadFriends();
    loadChats();
    loadGroups();
    showSection("chats");
...
window.saveProfile = async () => {
  try{
    ...
    alert("Profile saved & joined your batch group"); // Alert fixed
    showSection("chats");
    loadGroups(); // Refresh groups tab
  }catch(err){
...
// Find Classmates (only on search)
...
window.openGroupChat = async (groupId,name)=>{
  ...
  document.getElementById("chatName").innerText = name;
  showSection("chatScreen");
  // For group chat, fetch messages by groupId if implemented
}
